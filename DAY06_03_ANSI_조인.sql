...
/*
    ...
    2. 드리븐 테이블
        ...
        2) 일반적으로 데이터 갯수가 많다.
        3) FK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 사용이 가능하다.(느리다는 의미)
...
SELECT EMPLOYEE_ID
     ...
     , D.DEPARTMENT_ID
     , DEPARTMENT_NAME 
  FROM DEPARTMENTS D RIGHT OUTER JOIN EMPLOYEES E -- 오른쪽 테이블(EMPLOYEES)의 모든 데이터를 조회하시오. (사원이 근무하지 않는 부서도 조회하시오.) 
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
