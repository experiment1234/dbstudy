...
-- 5. EMPLOYEES 테이블에서 JOB_ID가 'IT_PROG', 'ST_MAN'인 사원을 조회하시오.
  SELECT *
    FROM EMPLOYEES
   WHERE JOB_ID IN('IT_PROG', 'ST_MAN'); -- DAY 5 (DQL)
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID
 ...
 ORDER BY TO_CHAR(SALARY, '99.999') DESC; -- 내답안 ( 99.999 : SALARY 5자리로 되어서)
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID
     , SALARY 
     , RANK() OVER(ORDER BY SALARY DESC) AS 부서내연봉순위
  FROM EMPLOYEES;  -- 내답안(왜 이렇게는 안되는지?)
