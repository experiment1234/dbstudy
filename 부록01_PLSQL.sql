/*  
     ...
     기초데이터 준비
     HR 계정의 EMPLYOEES 테이블을 GD계정으로 복사해서 사용
*/
DROP TABLE EMPLOYEES;
CREATE TABLE EMPLYOEES AS (
    SELECT *
      FROM HR.EMPLOYEES
);
...
/*
    ...
    변수 선언하기
    1. 값을 저장할 때 대입 연산자(:=)을 사용한다. 
    2. 타입을 선언하는 방식
        1) 스칼라 변수 : 타입을 직접 지정한다.
        2) 참조 변수   : 특정 칼럼의 타입을 그대로 사용한다. -- VARCHAR2라고 부르지 않고 저 칼럼의 타입을 쓰겠다고 함
*/
...
-- Hello World 출력하기
BEGIN 
    DBMS_OUTPUT.PUT_LINE('Hello World'); -- 문자와 문자열의구분이 없음
END;
...
DECLARE
    ...
    FISTNAME   EMPLOYEES.FIRST_NAME%TYPE;
