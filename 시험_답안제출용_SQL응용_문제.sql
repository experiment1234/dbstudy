...
--200       4
--100       8
SELECT DEPT_NO AS 학과별, COUNT(*) AS 학생수
  FROM STUDENT_T
 GROUP BY DEPT_NO;
...
SELECT D.DEPT_NO AS "학과번호", D.DEPT_NAME AS "학과명", NVL(COUNT(S.STUD_NO),0) AS 학생수
  FROM STUDENT_T S RIGHT OUTER JOIN DEPARTMENT_T D
...
SELECT STUD_NAME AS 학생명, DEPT_NAME AS 학과명
  FROM STUDENT_T S INNER JOIN DEPARTMENT_T D
  ON S.DEPT_NO = D.DEPT_NO;
...
--6     이재우  4200
--6     이만식  4200
SELECT   RANK() OVER(ORDER BY SUM(PROF_SAL) DESC) AS 순위
       , PROF_NAME AS 교수명
       , PROF_SAL AS 연봉
   FROM PROFESSOR_T
GROUP BY PROF_NAME, PROF_SAL;
