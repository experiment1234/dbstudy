...
CREATE INDEX CUST_NAME_IX ON CUST_T(CUST_NAME);

-- 5. 현재 접속한 사용자(USER)가 소유한 인덱스 정보를 가지고 있는 데이터 사전을 조회하여 CUST_T 테이블에 존재하는 인덱스의 이름과 해당 인덱스가 설정된 칼럼 이름을 조회하시오.(5점)
SELECT * FROM USER_IND_COLUMNS;
SELECT * FROM USER_IND_COLUMNS WHERE INDEX_NAME = 'CUST_NAME_IX' AND COLUMN_NAME = 'CUST_NAME';
...
-- BANK002    서울은행   02-321-9876
-- BANK003    경기은행   031-654-3210
SELECT ROWID, BANK_CODE, BANK_NAME, BANK_TEL
  FROM BANK_T;
...
SELECT B.BANK_CODE AS 은행코드
     , B.BANK_NAME AS 은행이름
     , B.BANK_TEL  AS 전화번호
     , COUNT(C.BANK_CODE) AS CUST_COUNT
  FROM BANK_T B LEFT OUTER JOIN CUST_T C
    ON C.BANK_CODE = B.BANK_CODE
GROUP BY B.BANK_CODE, B.BANK_NAME, B.BANK_TEL, C.BANK_CODE
