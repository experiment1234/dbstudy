...
--외래키 추가하기
ALTER TABLE PLAYER_TBL   ADD CONSTRAINT FK_NATION_PLAYER FOREIGN KEY(N_CODE) REFERENCES NATION_TBL(N_CODE) ON DELETE CASCADE;-- PLAYER_TBL의 N_CODE는 NOT NULL 이므로, ON DELETE SET NULL이 불가능하다. /함께 지우는 ON DELETE CASCADE 가능(삭제옵션에서 NULL값 불가능, 애초에 NOT NULL이라서) 
ALTER TABLE PLAYER_TBL   ADD CONSTRAINT FK_EVENT_PLAYER FOREIGN KEY(E_CODE) REFERENCES EVENT_TBL(E_CODE) ON DELETE CASCADE;-- PLAYER_TBL의 E_CODE는 NOT NULL 이므로, ON DELETE SET NULL이 불가능하다. 
ALTER TABLE SCHEDULE_TBL ADD CONSTRAINT FK_NATION_SCHEDULE FOREIGN KEY(N_CODE) REFERENCES NATION_TBL(N_CODE) ON DELETE CASCADE;--ON DELETE SET NULL이 가능하다.
ALTER TABLE SCHEDULE_TBL ADD CONSTRAINT FK_EVENT_SCHEDULE FOREIGN KEY(E_CODE) REFERENCES EVENT_TBL(E_CODE) ON DELETE CASCADE;--ON DELETE SET NULL이 가능하다./ ON DELETE SET CASCASE도 가능하다 (국가코드,종목  사라지면 NULL로 채워서 보관할 수 있음 or 함께 지우는 것이 가능함)
...
--선수 테이블의 FK_EVENT_PLAEYR 외래키 일시중지하기
ALTER TABLE PLAYER_TBL DISABLE CONSTRAINT FK_EVENT_PLAYER;
--선수 테이블의 FK_EVENT_PLAEYR 외래키 다시시작하기
