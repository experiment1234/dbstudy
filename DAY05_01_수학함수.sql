...
SELECT ABS(-5) -- -5의 절대값 5
  FROM DUAL; -- DUAL 아무 의미없는 테이블, 문법의 필요성에 의해서 추가함 
...
SELECT ROUND(123.456)    -- 123    (정수로 반올림)
     , ROUND(123.456, 1) -- 123.5  (소수 1자리로 반올림) , 56은 반올림 대상임
...
-- 9. 원하는 자릿수로 절사
SELECT TRUC(123.456)     -- 123    (정수로 절사)
     , TRUC(123.456, 1)  -- 123.4  (소수 1자리로 절사)
     , TRUC(123.456, 2)  -- 123.45 (소수 2자리로 절사) 
     , TRUC(123.456, -1) -- 120    (일의 자리에서 절사)
     , TRUC(123.456, -2) -- 100    (십의 자리에서 절사)
  FROM DUAL;
