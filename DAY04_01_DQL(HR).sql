...
-- 3. 사원 테이블에서 EMPOLYEE_ID가 150인 사원의 정보 조회하기
SELECT *
    FROM EMPLOYEES
...
SELECT *
    FROM EMPLOYEES
   WHERE DEPARTMENT_ID IN 30 OR DEPARTMENT_ID = 40 OR DEPARTMENT_ID = 50;
...
--10. 사원 테이블에서 전화번호가 '515'로 시작하는 전화번호의 중복을 제거해서 조회하기
...
SELECT *
    FROM EMPLOYEES
   ORDER BY SALARY DESC; -- DESC : Descendnig(내림차순) 
   
-- 12. 사원테이블의 사원들을 입사순으로 조회하기( 먼저 입사한 사원을 먼저 조회하기)
SELECT *
    FROM EMPLOYEES
...
-- 1차 정렬 기준 : 부서 , 오름차순(작은 부서 -> 높은 부서)

-- 2차 정렬 기준 : 연봉, 내림차순 

SELECT *
    FROM EMPLOYEES
   ORDER BY DEPARTMENT_ID, SALARY DESC;-- ASC 생략도 가능
