...
SELECT DEPARTMENT_ID    -- GROUP BY절에서 지정한 칼럼만 조회할 수 있다. 
     , COUNT(*)
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
 
-- 2. 사원 테이블에서 같은 직업을 가진 사원들을 그룹화화여 각 그룹별로 연봉의 평균이 얼마인지 조회하시오.
SELECT JOB_ID
     , ROUND(AVG(SALARY),2)
  FROM EMPLOYEES
 GROUP BY JOB_ID;
...
/* 
    ...
    3. GROUP BY 절이 필요하지 않는 조건은 WHERE절로 지정한다.  
*/
...
-- 5. 사원 테이블에서 각 부서별 사원수를 조회하시오. 단, 부서번호가 없는 사원은 제외하시오.(HAVING 안씀 - 성능떨어짐) 
-- 조건 : 부서번호 IS NOT  NULL
-- 조건에서 사용되는 부서번호는 GROUP BY절이 필요없으므로 WEHRE 절로 처리
SELECT DEPARTMENT_ID
     , COUNT(*)
  FROM EMPLOYEES   
